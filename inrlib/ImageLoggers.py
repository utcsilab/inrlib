...
class NeuralImplicitImageLogger(Callback):
    def __init__(self, view_transforms: List[ABCTransform]=[],  
                 save_freq: int=1, 
                 best_only=True, 
                 disabled=False,
                 **kwargs
    ...
    def _save_imgs(self, 
                   ...
                   **plotting_kwargs):
        save_imgs(imgs, save_path, labels=labels, metrics=metrics, method=method, **plotting_kwargs)
    ...
    def setup(self, trainer, pl_module: NeuralImplicitMLP, stage):
        ...
        assert isinstance(dataset, ABCDataset), "Dataset must be derived from ABCDataset"
        ...
        for transf_name in transf_orig:
            save_path = path.join(trainer.logger.log_dir, f"original_image_{transf_name}.png")
    ...
    def on_validation_end(self, trainer, pl_module: NeuralImplicitMLP):
        ...
        for transf_name in transf_out:
            ...
            fname = '_'.join( [state_fn, transf_name, f"e={trainer.current_epoch}_v={self.val_idx}"] ) + '.png'
            img_path = path.join(trainer.logger.log_dir, fname)
            self._save_imgs(save_imgs, img_path, labels=labels, metrics=scores, method='plt', **plotting_kwargs)
